...
purpose: Single-file, self-contained knowledge base for an AI chatbot to understand EVEFORGE, the ESI API, and the EVE SDE
---
...
under `knowledge/` in this repo, kept in sync with this file — use whichever is
more convenient for your context window.

## Who this is for

The operator plays EVE Online with 60+ characters and uses
**[eveforge.org](https://eveforge.org)** (EVEFORGE) as their industry & market ERP
— it does the actual number-crunching (BOM, margins, invention odds, PI
profitability, etc.). The problem this repo solves: EVEFORGE's layout and some of
its terminology aren't self-explanatory, and understanding what it does well
requires knowing what data it has access to (the EVE ESI API) and what static game
data underlies everything (the EVE SDE). This file gives an assistant all three at
once.
...
always be pulled fresh from EVEFORGE, ESI, or in-game — never asserted from memory
or from this file.

---

# Part 1 — EVEFORGE Site Map & Terminology
...
to target, build to cover the deficit" workflow — see Part 4 (Methodologies).
...
- `markets/{region_id}/orders/`, `/history/`, and `/types/` are **public** per
  ...
  (The Forge) — see the trade-hub → region ID table in Part 3.
...
`GET /industry/systems/`, public) × structure/facility tax settings. See Part 1's
...
- **Abyssal / Mutated modules.** These have no fixed stats in the SDE at all —
  ...
  (Part 2 → Dogma) returns the item's real, rolled attributes and **completely
...
- **The SDE has no prices.** `invTypes.basePrice` exists but is a vestigial
  ...
  come from ESI market endpoints (Part 2) or a market tool like EVEFORGE.
...
# Part 4 — Industrialist Methodologies
...
see Part 1 → Inventory → **Stockpiles** ("named inventory target containers for
...
`markets/{region_id}/orders/` endpoint in Part 2, are the right tools for that).
...
C-J6MT Keepstar, `structure_id 1049588174021` (Part 2 → Player-owned market
...
1049588174021** in Part 2 — a made-up or wrong ID would not return coherent live
pricing here.
...
budget (Part 2). Treat that as "no published limit" rather than "no limit" — keep
