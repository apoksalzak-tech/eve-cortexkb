...
purpose: Single-file, self-contained AI context repo distilling the operator's EVE Online knowledge over time — generic, tool-agnostic EVE knowledge first (the EVE ESI API, the EVE SDE, operator methodologies), plus per-tool deep-dives, active and archived
---
...
guessing. The focus is **generic, tool-agnostic EVE knowledge first**: two
foundational data sources — the **EVE ESI API** and the **EVE SDE** (Static Data
Export) — the operator's own methodologies (Part 3), active per-tool deep-dives
(Part 4), standalone practical references like public no-auth market data
lookups (Part 5), and archived-but-fully-documented tool deep-dives (Part 6).
**[eveforge.org](https://eveforge.org) (EVEFORGE)** is fully documented but
archived in Part 6 — it is not treated as the operator's only or permanent tool;
other tools (Eve Guru, POD, ...) will get their own active sections in Part 4 as
...
`GET /industry/systems/`, public) × structure/facility tax settings. See Part 6's
...
see Part 6 → Inventory → **Stockpiles** ("named inventory target containers for
...
live UI, for tools currently being adopted/documented. None of these is treated
as the operator's single primary tool — check whichever section matches the tool
actually in question. Both entries below are currently placeholders; see Part 6
for a previously fully-documented example (EVEFORGE) that's since been archived.
...
Fully documented tool deep-dives that are no longer the active focus, kept
rather than deleted. Content is unchanged from when it was written and
observed — archived means "not the current focus," not "stale" or
"off-limits." Check here if the tool named in a question isn't in Part 4.
